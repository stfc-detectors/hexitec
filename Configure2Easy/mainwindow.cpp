...
TwoEasyString *aspectIni, *twoEasyIni, *newportXpsIP, *newportXpsPort;

MainWindow::MainWindow(QWidget *parent) :
   QMainWindow(parent)
{
   createMenus();
   settings = new QSettings(QSettings::UserScope, "TEDDI", "2Easy");
   setWindowTitle(QString::fromUtf8("2Easy Configuration V1.3"));
   ...
   grid->addWidget(aspectIni = new TwoEasyString("aspectIniFilename", settings));
   grid->addWidget(twoEasyIni = new TwoEasyString("2EasyIniFilename", settings));
   grid->addWidget(newportXpsIP = new TwoEasyString("newportXPSIPaddress", settings));
   grid->addWidget(newportXpsPort = new TwoEasyString("newportXPSPort", settings));
...
void MainWindow::printStates()
{
   ...
   cout << "2Easy ini filename " << twoEasyIni->getText().toStdString() << endl;
