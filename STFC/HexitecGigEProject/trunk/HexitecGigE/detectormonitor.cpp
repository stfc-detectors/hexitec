...
DetectorMonitor::DetectorMonitor(AspectDetector *aspectDetector, char *portName, int loggingInterval, QObject *parent) :
   QObject(parent)
{
   ...
   this->aspectDetector = aspectDetector;
...
void DetectorMonitor::receiveState(AspectDetector::DetectorState detectorState)
{
   if (detectorState == AspectDetector::INITIALISED)
   {
      readTAsic = true;
      read();
   }
