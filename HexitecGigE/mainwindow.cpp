...
//           processingBufferGenerator, SLOT(handleConfigureSensor(int, int)));
   ...
   /// Debugging purposes: Calculate duration for processing
   qtTime = QTime();
   connect(processingForm, SIGNAL(processingHasBegun()), this, SLOT(startTimer()));
   connect(processingBufferGenerator, SIGNAL(processingHasStopped()), this, SLOT(stopTimer()));
}
...
{
   qtTime.restart();
   timeElapsed = qtTime.elapsed();
}

void MainWindow::stopTimer()
{
   qDebug() << "Processing time taking:" << (qtTime.elapsed() - timeElapsed);
}
